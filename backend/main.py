...
async def lifespan(app: FastAPI):
    ...
    verify_database_config(settings)
    ...
    if should_create_all(settings):
        try:
            ...
            logger.error("Database connection failed for host %r: %s", settings.database_host, exc)
            raise RuntimeError(
                f"Database connection failed for host {settings.database_host!r}: {exc}. "
                "If you are deploying to Render, please set the following environment variables in the Render Dashboard (Environment tab):\n"
                "  1. DATABASE_URL = <your pooled Neon connection string (e.g. postgresql+asyncpg://...neon.tech/...)>\n"
